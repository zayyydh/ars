...
class ScreeningResult:
    ...
    def summary(self) -> str:
        bar_filled = int(self.total_score / 5)
        bar = "█" * bar_filled + "░" * (20 - bar_filled)
        return (
            f"\n  Candidate : {self.candidate_name or 'Unknown'}\n"
            f"  Role      : {self.job_title or 'Unknown'}\n"
            f"  Score     : [{bar}] {self.total_score:.1f}/100\n"
            f"  Verdict   : {self.verdict.value}\n"
            f"  Skills    : {self.skill_score:.1f}  "
            f"Exp: {self.experience_score:.1f}  "
            f"Edu: {self.education_score:.1f}\n"
            f"  Matched   : {len(self.matched_skills)} | "
            f"Missing: {len(self.missing_skills)} | "
            f"Gap: {self.exp_gap:+.1f} yrs"
        )