...
Uses regex-based extraction (LLM disabled).
"""
...
def parse(jd_text: str) -> JobRequirements:
    """Parse a job description using regex. No LLM required."""
...
def _parse_with_regex(jd_text: str) -> JobRequirements:
    ...
    # Skills — split into required vs preferred based on context
    required, preferred = [], []
    ...
    logger.info(
        f"jd_parser: found {len(required)} required + "
        f"{len(preferred)} preferred skills, "
